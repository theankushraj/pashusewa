...
// Default HTML response for non-API routes when ASSETS binding is not available
function defaultIndexHtml() {
  ...
  <html>
    <head>
      ...
      <title>PashuSewa</title>
      <style>
        body { font-family: sans-serif; max-width: 800px; margin: 0 auto; padding: 20px; }
        h1 { color: #4caf50; }
        .message { padding: 20px; background-color: #f9f9f9; border-radius: 8px; }
      </style>
    </head>
    <body>
      <h1>PashuSewa API</h1>
      <div class="message">
        <p>The PashuSewa API is running. To use the application, please serve the static files properly.</p>
        <p>Available endpoints:</p>
        <ul>
          <li>POST /api/report - Submit a new animal report</li>
          <li>GET /api/reports - Get all reports</li>
          <li>POST /api/update-status - Update the status of a report</li>
        </ul>
      </div>
    </body>
  </html>`, {
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      // For Worker Sites, the binding is __STATIC_CONTENT
      if (env.__STATIC_CONTENT) {
        return env.__STATIC_CONTENT.fetch(request);
      } else if (env.ASSETS) {
        // Fallback to ASSETS binding if configured
        return env.ASSETS.fetch(request);
      } else {
        console.warn('Static content bindings not configured, serving default page');
